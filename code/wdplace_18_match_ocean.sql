...
drop table if exists  wfwd.wd_match_ocean CASCADE;
CREATE UNLOGGED TABLE          wfwd.wd_match_ocean  as
with x AS (
        select
            wd_id
...
CREATE INDEX  ON  wfwd.wd_match_ocean USING GIST(wd_point_merc);
CREATE INDEX  ON  wfwd.wd_match_ocean (una_wd_name_en_clean);
CREATE INDEX  ON  wfwd.wd_match_ocean (wd_id);
CREATE INDEX  ON  wfwd.wd_match_ocean USING GIN(wd_name_array );
CREATE INDEX  ON  wfwd.wd_match_ocean USING GIN(wd_altname_array );
ANALYSE   wfwd.wd_match_ocean ;




drop table if exists wfwd.wof_match_ocean CASCADE;
CREATE UNLOGGED TABLE         wfwd.wof_match_ocean  as
select
     wof.id
...
from wf.wof_ocean as wof
where  wof.is_superseded=0  and wof.is_deprecated=0
;

CREATE INDEX  ON wfwd.wof_match_ocean  USING GIST(wof_geom_merc);
CREATE INDEX  ON wfwd.wof_match_ocean  (una_wof_name);
CREATE INDEX  ON wfwd.wof_match_ocean  USING GIN ( wof_name_array);
ANALYSE          wfwd.wof_match_ocean ;
...
\set wd_input_table           wfwd.wd_match_ocean
\set wof_input_table          wfwd.wof_match_ocean

\set wd_wof_match             wfwd.wd_mocean_wof_match
...
\set safedistance   999000000
\set searchdistance 999000003
