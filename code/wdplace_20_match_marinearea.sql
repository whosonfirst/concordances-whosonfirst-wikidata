...
drop table if exists  wfwd.wd_match_marinearea CASCADE;
CREATE UNLOGGED TABLE          wfwd.wd_match_marinearea  as
with x AS (
        select
            wd_id
        ...
        from wd.wdx 
        where a_wof_type && ARRAY['marinearea']    
    )
    SELECT *
...
CREATE INDEX  ON  wfwd.wd_match_marinearea USING GIST(wd_point_merc);
--CREATE INDEX  ON  wfwd.wd_match_marinearea (una_wd_name_en_clean);
--CREATE INDEX  ON  wfwd.wd_match_marinearea (wd_id);
--CREATE INDEX  ON  wfwd.wd_match_marinearea USING GIN(wd_name_array );
--CREATE INDEX  ON  wfwd.wd_match_marinearea USING GIN(wd_altname_array );
--ANALYSE   wfwd.wd_match_marinearea ;




drop table if exists wfwd.wof_match_marinearea CASCADE;
CREATE UNLOGGED TABLE         wfwd.wof_match_marinearea  as
select
     wof.id
...
from wf.wof_marinearea as wof
where  wof.is_superseded=0  and wof.is_deprecated=0
;

CREATE INDEX  ON wfwd.wof_match_marinearea  USING GIST(wof_geom_merc);
--CREATE INDEX  ON wfwd.wof_match_marinearea  (una_wof_name);
--CREATE INDEX  ON wfwd.wof_match_marinearea  USING GIN ( wof_name_array);
--ANALYSE          wfwd.wof_match_marinearea ;
...
\set wd_input_table           wfwd.wd_match_marinearea
\set wof_input_table          wfwd.wof_match_marinearea

\set wd_wof_match             wfwd.wd_mmarinearea_wof_match
\set wd_wof_match_agg         wfwd.wd_mmarinearea_wof_match_agg
\set wd_wof_match_agg_sum     wfwd.wd_mmarinearea_wof_match_agg_summary
\set wd_wof_match_notfound    wfwd.wd_mmarinearea_wof_match_notfound
\set safedistance   9000000
\set searchdistance 9000003
