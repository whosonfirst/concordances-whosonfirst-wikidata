...
drop table if exists  wfwd.wd_match_macrohood CASCADE;
CREATE UNLOGGED TABLE          wfwd.wd_match_macrohood  as
with x AS (
        select
            wd_id
...
CREATE INDEX  ON  wfwd.wd_match_macrohood USING GIST(wd_point_merc);
CREATE INDEX  ON  wfwd.wd_match_macrohood (una_wd_name_en_clean);
CREATE INDEX  ON  wfwd.wd_match_macrohood (wd_id);
CREATE INDEX  ON  wfwd.wd_match_macrohood USING GIN(wd_name_array );
CREATE INDEX  ON  wfwd.wd_match_macrohood USING GIN(wd_altname_array );
ANALYSE   wfwd.wd_match_macrohood ;




drop table if exists wfwd.wof_match_macrohood CASCADE;
CREATE UNLOGGED TABLE         wfwd.wof_match_macrohood  as
select
     wof.id
...
from wf.wof_macrohood as wof
where  wof.is_superseded=0  and wof.is_deprecated=0
;

CREATE INDEX  ON wfwd.wof_match_macrohood  USING GIST(wof_geom_merc);
CREATE INDEX  ON wfwd.wof_match_macrohood  (una_wof_name);
CREATE INDEX  ON wfwd.wof_match_macrohood  USING GIN ( wof_name_array);
ANALYSE          wfwd.wof_match_macrohood ;
...
\set wd_input_table           wfwd.wd_match_macrohood
\set wof_input_table          wfwd.wof_match_macrohood

\set wd_wof_match             wfwd.wd_mmacrohood_wof_match
\set wd_wof_match_agg         wfwd.wd_mmacrohood_wof_match_agg
\set wd_wof_match_agg_sum     wfwd.wd_mmacrohood_wof_match_agg_summary
\set wd_wof_match_notfound    wfwd.wd_mmacrohood_wof_match_notfound
