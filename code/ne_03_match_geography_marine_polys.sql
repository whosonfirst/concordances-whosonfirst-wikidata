...
drop table if exists                    newd.wd_match_geography_marine_polys CASCADE;
EXPLAIN ANALYSE CREATE UNLOGGED TABLE   newd.wd_match_geography_marine_polys  as
with x as
(
    select
         wd_id
      ...
      --  ,get_wd_concordances(data)         as wd_concordances_array
        ...
        ,a_wof_type 
    from wd.wdx 
    where (a_wof_type  &&  ARRAY['marinearea','ocean'] ) 
)
select * 
from x
where (not wd_is_cebuano ) and (a_wof_type  @> ARRAY['hasP625'] ) 
;

CREATE INDEX  ON  newd.wd_match_geography_marine_polys USING GIST(wd_point_merc);
CREATE INDEX  ON  newd.wd_match_geography_marine_polys (wd_id);
ANALYSE           newd.wd_match_geography_marine_polys ;
...
CREATE INDEX  ON newd.ne_match_geography_marine_polys  USING GIST(ne_geom_merc);
ANALYSE          newd.ne_match_geography_marine_polys;

\set wd_input_table           newd.wd_match_geography_marine_polys
\set ne_input_table           newd.ne_match_geography_marine_polys

\set ne_wd_match               newd.ne_wd_match_geography_marine_polys_match
\set ne_wd_match_agg           newd.ne_wd_match_geography_marine_polys_match_agg
\set ne_wd_match_agg_sum       newd.ne_wd_match_geography_marine_polys_match_agg_sum
\set ne_wd_match_notfound      newd.ne_wd_match_geography_marine_polys_match_notfound
