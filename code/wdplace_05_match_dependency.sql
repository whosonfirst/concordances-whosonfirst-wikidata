...
drop table if exists  wd_mdependency_wof_match CASCADE;
create table          wd_mdependency_wof_match  as
    select
         wof.id 
        ,wof.wof_name
        ,wof.wof_country
        ...
        ,wd.*        
    from wdplace.wd_match_dependency  as wd 
...
drop table if exists  wd_mdependency_wof_match_agg CASCADE;
create table          wd_mdependency_wof_match_agg  as
with wd_agg as 
(
    select id, wof_name, wof_country,wof_wd_id
        ,  array_agg(wd_id     order by     wd_id) as a_wd_id       
    from wd_mdependency_wof_match
...
left join wdplace.wd_match_dependency as wdnew  on wd_agg_extended._suggested_wd_id=wdnew.wd_id   
;
ANALYSE wd_mdependency_wof_match_agg ;


drop table if exists  wd_mdependency_wof_match_agg_summary CASCADE;
create table          wd_mdependency_wof_match_agg_summary  as
    select _matching_category,  wd_number_of_matches,  count(*) as N  
    from wd_mdependency_wof_match_agg
