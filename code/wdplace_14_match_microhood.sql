...
drop table if exists  wfwd.wd_match_microhood CASCADE;
CREATE UNLOGGED TABLE          wfwd.wd_match_microhood  as
with x AS (
        select
            wd_id
...
CREATE INDEX  ON  wfwd.wd_match_microhood USING GIST(wd_point_merc);
CREATE INDEX  ON  wfwd.wd_match_microhood (una_wd_name_en_clean);
CREATE INDEX  ON  wfwd.wd_match_microhood (wd_id);
CREATE INDEX  ON  wfwd.wd_match_microhood USING GIN(wd_name_array );
CREATE INDEX  ON  wfwd.wd_match_microhood USING GIN(wd_altname_array );
ANALYSE   wfwd.wd_match_microhood ;




drop table if exists wfwd.wof_match_microhood CASCADE;
CREATE UNLOGGED TABLE         wfwd.wof_match_microhood  as
select
     wof.id
...
from wf.wof_microhood as wof
where  wof.is_superseded=0  and wof.is_deprecated=0
;

CREATE INDEX  ON wfwd.wof_match_microhood  USING GIST(wof_geom_merc);
CREATE INDEX  ON wfwd.wof_match_microhood  (una_wof_name);
CREATE INDEX  ON wfwd.wof_match_microhood  USING GIN ( wof_name_array);
ANALYSE          wfwd.wof_match_microhood ;
...
\set wd_input_table           wfwd.wd_match_microhood
\set wof_input_table          wfwd.wof_match_microhood

\set wd_wof_match             wfwd.wd_mmicrohood_wof_match
\set wd_wof_match_agg         wfwd.wd_mmicrohood_wof_match_agg
\set wd_wof_match_agg_sum     wfwd.wd_mmicrohood_wof_match_agg_summary
\set wd_wof_match_notfound    wfwd.wd_mmicrohood_wof_match_notfound
