...
CREATE OR REPLACE FUNCTION  airports_clean(name text) 
    RETURNS text  
...
EXPLAIN ANALYSE CREATE UNLOGGED TABLE   newd.wd_match_airports  as
select
     wd_id
    ,wd_label               as wd_name_en
    ,check_number(wd_label) as wd_name_has_num
    ,          (regexp_split_to_array( wd_label , '[,()]'))[1]  as wd_name_en_clean
    ,airports_clean((regexp_split_to_array( wd_label , '[,()]'))[1]) as una_wd_name_en_clean
...
CREATE INDEX  ON  newd.wd_match_airports USING GIST(wd_point_merc);
CREATE INDEX  ON  newd.wd_match_airports (wd_id);
...
select
    ...
    ,featurecla   
    ,name                 as ne_name
    ,airports_clean(name) as ne_una_name        
...
CREATE INDEX  ON newd.ne_match_airports  USING GIST(ne_geom_merc);
ANALYSE          newd.ne_match_airports;

\set wd_input_table           newd.wd_match_airports
\set ne_input_table           newd.ne_match_airports

\set ne_wd_match               newd.ne_wd_match_airports_match
\set ne_wd_match_agg           newd.ne_wd_match_airports_match_agg
\set ne_wd_match_agg_sum       newd.ne_wd_match_airports_match_agg_sum
\set ne_wd_match_notfound      newd.ne_wd_match_airports_match_notfound
\set safedistance    40000
\set searchdistance 400003
\set suggestiondistance  80000

\set mcond1   (  (  wd.wd_iata =ne.iata_code) or  ( ne.ne_una_name = wd.una_wd_name_en_clean ) or (  wd_name_array && ne_name_array ) or (  ne_name_array && wd_altname_array )  or (jarowinkler( ne.ne_una_name, wd.una_wd_name_en_clean)>.971 ) )
