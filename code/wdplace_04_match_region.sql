...
drop table if exists  wd_mregion_wof_match CASCADE;
create table          wd_mregion_wof_match  as
    select
         wof.id 
        ,wof.wof_name
        ,wof.wof_country
        ...
        ,wd.*        
    from wdplace.wd_match_region  as wd 
        ,wof_match_region         as wof
    where      wof.wof_country  = wd.wd_country 
           and wof.una_wof_name = wd.una_wd_name_en_clean
;
ANALYSE     wd_mregion_wof_match ;



drop table if exists  wd_mregion_wof_match_agg CASCADE;
create table          wd_mregion_wof_match_agg  as
with wd_agg as 
(
    select id, wof_name, wof_country,wof_wd_id
        ,  array_agg(wd_id     order by     wd_id) as a_wd_id       
    from wd_mregion_wof_match
...
left join wdplace.wd_match_region  as wdnew  on wd_agg_extended._suggested_wd_id=wdnew.wd_id   
;
ANALYSE wd_mregion_wof_match_agg ;


drop table if exists  wd_mregion_wof_match_agg_summary CASCADE;
create table          wd_mregion_wof_match_agg_summary  as
    select _matching_category,  wd_number_of_matches,  count(*) as N  
    from wd_mregion_wof_match_agg
    group by  _matching_category, wd_number_of_matches
    order by  _matching_category, wd_number_of_matches
    ;
ANALYSE wd_mregion_wof_match_agg_summary ;



drop table if exists wd_mregion_wof_notfound CASCADE;
create table         wd_mregion_wof_notfound  as
select
     wof.id
    ,wof.wof_name 
    ,wof.wof_country
    ,wof.wof_wd_id
from wof_match_region as wof
where  wof.id not in ( select distinct id from wd_mregion_wof_match )  
order by wof.wof_country, wof.wof_name
;
ANALYSE wd_mregion_wof_notfound;
