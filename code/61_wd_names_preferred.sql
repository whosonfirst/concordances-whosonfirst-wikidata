...
with wd_names_iso2 as
 ...
 select
    ...
    ,(regexp_split_to_array( data->'labels'->jsonb_object_keys(data->'labels')->>'value'  , '[,()]'))[1]   as wof_value
    FROM wd.wdx
 )
select
       wd.wd_id
...
where wd.wd_lang != 'ceb'    
    and length(coalesce( langcodes.alpha3_b ,wd.wd_lang )) = 3    
order by wd_id
;
