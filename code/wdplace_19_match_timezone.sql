...
drop table if exists  wfwd.wd_match_timezone CASCADE;
CREATE UNLOGGED TABLE          wfwd.wd_match_timezone  as
with x AS (
        select
            wd_id
        ...
        from wd.wdx 
        where a_wof_type && ARRAY['timezone']    
    )
    SELECT *
...
CREATE INDEX  ON  wfwd.wd_match_timezone USING GIST(wd_point_merc);
CREATE INDEX  ON  wfwd.wd_match_timezone (una_wd_name_en_clean);
CREATE INDEX  ON  wfwd.wd_match_timezone (wd_id);
CREATE INDEX  ON  wfwd.wd_match_timezone USING GIN(wd_name_array );
CREATE INDEX  ON  wfwd.wd_match_timezone USING GIN(wd_altname_array );
ANALYSE   wfwd.wd_match_timezone ;




drop table if exists wfwd.wof_match_timezone CASCADE;
CREATE UNLOGGED TABLE         wfwd.wof_match_timezone  as
select
     wof.id
...
from wf.wof_timezone as wof
where  wof.is_superseded=0  and wof.is_deprecated=0
;

CREATE INDEX  ON wfwd.wof_match_timezone  USING GIST(wof_geom_merc);
CREATE INDEX  ON wfwd.wof_match_timezone  (una_wof_name);
CREATE INDEX  ON wfwd.wof_match_timezone  USING GIN ( wof_name_array);
ANALYSE          wfwd.wof_match_timezone ;
...
\set wd_input_table           wfwd.wd_match_timezone
\set wof_input_table          wfwd.wof_match_timezone

\set wd_wof_match             wfwd.wd_mtimezone_wof_match
\set wd_wof_match_agg         wfwd.wd_mtimezone_wof_match_agg
\set wd_wof_match_agg_sum     wfwd.wd_mtimezone_wof_match_agg_summary
\set wd_wof_match_notfound    wfwd.wd_mtimezone_wof_match_notfound
